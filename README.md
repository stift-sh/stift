...
| `STIFT_LOGIN_PROVIDER` | server | sign-in adapter the web app hands off to, advertised on `/api/version` (`auth.login`); needs `jwt` in `STIFT_AUTH`. `clerk` loads Clerk's sign-in and organization switcher from your Clerk instance (self-host bundles never fetch it); `test` takes ready-made JWTs from the page and exists for end-to-end tests. Unset (default) = the login screen asks for an API token, and with `local` in `STIFT_AUTH` that form stays available next to the provider |
| `STIFT_CLERK_PUBLISHABLE_KEY`, `STIFT_CLERK_JWT_TEMPLATE` | server | with `STIFT_LOGIN_PROVIDER=clerk`: the instance's publishable key (required), and the name of the JWT template that carries the org claims above (unset = Clerk's default session token) |
| `STIFT_CLOUD_API_URL` | server | base URL of an operator's billing service, advertised on `/api/version`; with feature `cloud` the web app's Billing screen reads `GET /billing/status` from it and sends admins to the URL returned by `POST /billing/checkout` or `/billing/portal`, all with the signed-in user's bearer |
