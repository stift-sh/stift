# stift server image: the TypeScript API server (and, once apps/web exists,
# the built web bundle). Self-host with docker-compose.yml or:
#   docker run -p 8580:8580 -e STIFT_DATABASE_URL=... -e STIFT_S3_* ghcr.io/stift-sh/stift
FROM node:22-alpine AS build
RUN corepack enable
WORKDIR /src
COPY . .
RUN pnpm install --frozen-lockfile --filter "@stift/server..."
RUN pnpm turbo run build --filter=@stift/server
RUN pnpm --filter @stift/server deploy --prod --legacy /out

FROM node:22-alpine
ARG VERSION=dev
ENV NODE_ENV=production PORT=8580 STIFT_VERSION=${VERSION}
WORKDIR /app
